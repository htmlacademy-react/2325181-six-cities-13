...
Приложение состоит из нескольких страниц: Main, Login, Favorites (приватная), Offer.
...
Отзывы по умолчанию отсортированы от новых к старым (новые сверху).
...
При нажатии кнопки «Submit» и отправке данных кнопка «Submit» и вся форма блокируются. Разблокировка формы и кнопки происходит в случае успешной отправки или при возникновении ошибки.

В случае успешной отправки отзыва форма очищается.

В случае возникновения ошибки пользователь уведомляется посредством всплывающего сообщения.
...
Данные для страницы «Favorites» загружаются с сервера. Для этого предусмотрен отдельный маршрут (см. раздел «Взаимодействие с сервером»).
...
3. Дополнительно
Код проекта покрыт тестами. Есть тесты для всех компонентов, редьюсеров, асинхронных операций.
